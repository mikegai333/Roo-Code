import { StatusBarItem, WebviewView, commands, window, workspace, ExtensionContext } from "vscode"
...
export class ChatService {
	...
	private getProvider = () => {
		const provider = this._extensionContext?.globalState.get<TwinnyProvider>(ACTIVE_CHAT_PROVIDER_STORAGE_KEY)
		return provider
	}
	...
	private onStreamData = (streamResponse: StreamResponse | undefined, onEnd?: (completion: string) => void) => {
		const provider = this.getProvider()
		if (!provider) return
	...
	private onStreamEnd = (onEnd?: (completion: string) => void) => {
		...
		commands.executeCommand("setContext", EXTENSION_CONTEXT_NAME.twinnyGeneratingText, false)
		if (onEnd) {
	...
	private onStreamStart = (controller: AbortController) => {
		...
		commands.executeCommand("setContext", EXTENSION_CONTEXT_NAME.twinnyGeneratingText, true)
	...
	private buildMesageRoleContent = async (
		...
		const systemMessage = {
			role: "system",
			content: await this._templateProvider?.readSystemMessageTemplate(this._promptTemplate),
		}
	...
	private buildChatPrompt = async (messages: Message[]) => {
		...
		const selection = editor?.selection
		const selectionContext = editor?.document.getText(selection) || ""
		const prompt = await this._templateProvider?.renderTemplate<ChatTemplateData>("chat", {
	...
	private buildTemplatePrompt = async (template: string, language: CodeLanguageDetails, context?: string) => {
		const editor = window.activeTextEditor
		const selection = editor?.selection
		const selectionContext = editor?.document.getText(selection) || context || ""
		const prompt = await this._templateProvider?.renderTemplate<TemplateData>(template, {
			code: selectionContext || "",
	...
	private streamResponse({
		...
		return streamResponse({
			...
			onData: (streamResponse: StreamResponse | undefined) => this.onStreamData(streamResponse, onEnd),
	...
	public async streamTemplateCompletion(
		...
		this.sendEditorLanguage()
		const { prompt, selection } = await this.buildTemplatePrompt(promptTemplate, language, context)
