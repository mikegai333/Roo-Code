import { FIM_TEMPLATE_FORMAT, STOP_DEEPSEEK, STOP_LLAMA, STOP_STARCODER, STOP_CODEGEMMA } from "../common/constants"
import { supportedLanguages } from "../common/languages"
import { FimPromptTemplate } from "../common/types"

const getFileContext = (fileContextEnabled: boolean, context: string, language: string | undefined, header: string) => {
	const languageId = supportedLanguages[language as keyof typeof supportedLanguages]
...
export const getFimPromptTemplateCodegemma = ({ prefixSuffix }: FimPromptTemplate) => {
	const { prefix, suffix } = prefixSuffix
	return `<|fim_prefix|>${prefix}<|fim_suffix|>${suffix}<|fim_middle|>`
}

function getFimTemplateAuto(fimModel: string, args: FimPromptTemplate) {
	if (fimModel.includes(FIM_TEMPLATE_FORMAT.codellama) || fimModel.includes(FIM_TEMPLATE_FORMAT.llama)) {
	...
	if (fimModel.includes(FIM_TEMPLATE_FORMAT.stableCode) || fimModel.includes(FIM_TEMPLATE_FORMAT.starcoder)) {
...
export const getFimPrompt = (fimModel: string, format: string, args: FimPromptTemplate) => {
...
export const getStopWordsChosen = (format: string) => {
	...
	if (format === FIM_TEMPLATE_FORMAT.stableCode || format === FIM_TEMPLATE_FORMAT.starcoder) return STOP_STARCODER
