...
export class ConversationHistory {
	...
	setUpEventListeners() {
		this._webviewView.webview.onDidReceiveMessage((message: ClientMessage<Conversation>) => {
	...
	private getProvider = () => {
		return this._context?.globalState.get<TwinnyProvider>(ACTIVE_CHAT_PROVIDER_STORAGE_KEY)
	}
	...
	getConversations(): Conversations {
		const conversations = this._context.globalState.get<Record<string, Conversation>>(CONVERSATION_STORAGE_KEY)
	...
	setActiveConversation(conversation: Conversation | undefined) {
		this._context.globalState.update(ACTIVE_CONVERSATION_STORAGE_KEY, conversation)
		this._webviewView?.webview.postMessage({
	...
	getActiveConversation() {
		const conversation: Conversation | undefined = this._context.globalState.get(ACTIVE_CONVERSATION_STORAGE_KEY)
		this.setActiveConversation(conversation)
		return conversation
	}
