...
export async function streamResponse(request: StreamRequest) {
	...
	const config = workspace.getConfiguration("aixcoding.main.config")

	try {
		const url = `${options.protocol}://${options.hostname}${options.path}`
		console.log("20250218", url, body)
		const fetchOptions = {
		...
		while (true) {
			if (signal.aborted) {
				console.log("aborted executed", Date.now())
				break
			}
		...
		if (error instanceof Error) {
			if (error.name === "AbortError") {
				console.log("abortError123", error)
				onEnd?.()
			} else {
