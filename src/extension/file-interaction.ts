...
export class FileInteractionCache {
	...
	resetInactivityTimeout() {
		...
		this._inactivityTimeout = setTimeout(() => this.pauseSession(), this._inactivityThreshold)
	}
	...
	resumeSession() {
		...
		const pausedDuration = new Date().getTime() - this._sessionPauseTime.getTime()
		this._sessionStartTime = new Date(this._sessionStartTime.getTime() + pausedDuration)
	...
	incrementStrokes(currentLine: number, currentCharacter: number) {
		...
		this._interactions.set(this._currentFile, {
			...
			activeLines: [...item.activeLines, { line: currentLine, character: currentCharacter }],
		})
	...
	endSession(): void {
		...
		const sessionLength = (sessionEndTime.getTime() - this._sessionStartTime.getTime()) / 1000
