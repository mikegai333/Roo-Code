...
export class TemplateProvider {
	...
	public readSystemMessageTemplate(templateName?: string) {
		...
		try {
			return new Promise<string>((resolve, reject) => {
				fs.readFile(path, { encoding: "utf-8" }, (err, templateString: string) => {
	...
	public async renderTemplate<T extends DefaultTemplate>(templateName: string, data: T) {
		try {
			const template: HandlebarsTemplateDelegate<T> = await this.compileTemplateFromFile(templateName)
