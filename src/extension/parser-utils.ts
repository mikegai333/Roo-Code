...
export const getParser = async (filePath: string): Promise<Parser | undefined> => {
	...
	const wasmPath = path.join(__dirname, "tree-sitter-wasms", `tree-sitter-${language}.wasm`)
...
export function getNodeAtPosition(tree: Parser.Tree | undefined, position: Position): SyntaxNode | null {
