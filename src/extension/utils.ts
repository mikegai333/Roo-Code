...
export const getSkipVariableDeclataion = (characterBefore: string, textAfter: string) => {
	const con1 = (!textAfter.at(0) as unknown as string) === "?"
	const con2 = !getIsOnlyBrackets(textAfter)
	if (
		characterBefore &&
...
export const triggerCompletion = () => {
	...
	return !charAfter || getIsClosingBracket(charAfter)
}
...
export const getChatDataFromProvider = (provider: string, data: StreamResponse | undefined) => {
	switch (provider) {
		case ApiProviders.Ollama:
		case ApiProviders.OpenWebUI:
			return data?.choices[0].delta?.content ? data?.choices[0].delta.content : ""
...
export function safeParseJsonResponse(stringBuffer: string): StreamResponse | undefined {
	try {
