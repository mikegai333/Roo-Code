...
export class DebugRecorder extends Disposable {
	...
	constructor(
		...
		mapObservableArrayCached(
			...
			(doc, store) => {
				...
				const state = new DocumentHistory(
					root,
					doc.id,
					doc.value.get().value,
					this._id++,
	...
	public getTimestamp(): number {
		...
		if (this._lastTimestamp !== undefined && newTimestamp <= this._lastTimestamp) {
			// we want total ordering on the events
	...
	public getRecentLog(bookmark: DebugRecorderBookmark | undefined = undefined): LogEntry[] | undefined {
		if (!this._workspaceRoot) {
			// possible if the open file doesn't belong to a workspace
			return undefined
		}
		...
		log.push({
			entry: {
				...
				kind: "header",
				repoRootUri: this._workspaceRoot.toString(),
				time: this.getNow(),
				uuid: generateUuid(),
			},
			sortTime: 0,
...
class DocumentHistory {
	...
	private readonly relativePath = (() => {
		...
		return this.docId.toUri().scheme === Schemas.vscodeNotebookCell
			? `${basePath}#${this.docId.fragment}`
			: basePath
	})()
	...
	getDocumentLog(bookmark: DebugRecorderBookmark | undefined): { entry: LogEntry; sortTime: number }[] {
		...
		log.push({
			entry: {
				...
				id: this.id,
				v: docVersion,
				content: this._baseValue.value,
				time: this._baseValueTime,
			},
			sortTime: this._baseValueTime,
		})
		...
		for (const editOrSelectionChange of this._edits) {
			...
			if (editOrSelectionChange.kind === "selections") {
				const serializedOffsetRange: ISerializedOffsetRange[] = editOrSelectionChange.selections.map((s) => [
					s.start,
					s.endExclusive,
				])
				log.push({
					entry: {
						...
						id: this.id,
						selection: serializedOffsetRange,
						time: editOrSelectionChange.instant,
					},
					sortTime: editOrSelectionChange.instant,
				})
			} else {
				log.push({
					entry: {
						...
						id: this.id,
						v: docVersion,
						edit: serializeStringEdit(editOrSelectionChange.edit),
						time: editOrSelectionChange.instant,
					},
					sortTime: editOrSelectionChange.instant,
				})
