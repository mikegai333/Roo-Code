...
class WorkspaceInformation {
	...
	async updateNodeModules(): Promise<void> {
		...
		const workspaceNodeModules = await Promise.all(
...
export class ImportDiagnosticCompletionProvider
	implements IDiagnosticCompletionProvider<ImportDiagnosticCompletionItem>
{
	...
	async provideDiagnosticCompletionItem(
	...
	): Promise<ImportDiagnosticCompletionItem | null> {
		const language = workspaceDocument.languageId.get()
		const importDiagnosticToFix = sortedDiagnostics.find((diagnostic) =>
			this.providesCompletionsForDiagnostic(diagnostic, language, pos),
		)
		...
		if (availableCodeActions === undefined) {
			log(
				`Fetching code actions likely timed out for \`${importDiagnosticToFix.message}\``,
				logContext,
				this._tracer,
			)
			return null
		...
		log(
			`Resolving code actions for \`${importDiagnosticToFix.message}\` took \`${resolveCodeActionDuration}ms\``,
			logContext,
			this._tracer,
		)

		const availableImportCodeActions = this._getImportCodeActions(
		...
		logList(
			`Sorted import code actions for \`${importDiagnosticToFix.message}\``,
			sortedImportCodeActions,
			logContext,
			this._tracer,
		)

		for (const codeAction of sortedImportCodeActions) {
			const importCodeActionLabel =
				availableImportCodeActions.length === 1 && codeAction.importSource !== ImportSource.external
					? codeAction.labelShort
					: codeAction.labelDeduped

			const item = new ImportDiagnosticCompletionItem(
				codeAction,
				importDiagnosticToFix,
				importCodeActionLabel,
				workspaceDocument,
				availableImportCodeActions.length - 1,
			)

			if (this._hasImportBeenRejected(item)) {
				log(
					`Rejected import completion item ${codeAction.labelDeduped} for ${importDiagnosticToFix.toString()}`,
					logContext,
					this._tracer,
				)
			...
			log(
				`Created import completion item ${codeAction.labelDeduped} for ${importDiagnosticToFix.toString()}`,
				logContext,
				this._tracer,
			)
	...
	private _getImportCodeActions(
		codeActions: CodeAction[],
		workspaceDocument: IVSCodeObservableDocument,
		diagnostic: Diagnostic,
		workspaceInfo: WorkspaceInformation,
	): ImportCodeAction[] {
...
class JavascriptImportHandler implements ILanguageImportHandler {
	...
	private static ImportsToIgnore = new Set<string>([
		"type",
		"namespace",
		"module",
		"declare",
	...
	getImportDetails(
		...
		const importTitlePrefix = JavascriptImportHandler.CodeActionTitlePrefixes.find((prefix) =>
			codeAction.title.startsWith(prefix),
		)
	...
	getImportDetails(
		...
		if (fromImportMatch) {
			...
			return {
				...
				labelShort: `import ${importName}`,
				importSource: this._getImportSource(importPath),
			}
		}
		...
		if (importAsMatch) {
			...
			return {
				...
				labelDeduped: `import ${importName} as ${importAs}`,
				labelShort: `import ${importName} as ${importAs}`,
		...
		if (importMatch) {
			...
			return {
				importName,
				importPath: importName,
				labelDeduped: `import ${importName}`,
				labelShort: `import ${importName}`,
				importSource: ImportSource.unknown,
			}
		}
