...
export abstract class DiagnosticCompletionItem implements vscode.InlineCompletionItem {
	static equals(a: DiagnosticCompletionItem, b: DiagnosticCompletionItem): boolean {
		return (
			a.documentId.toString() === b.documentId.toString() &&
	...
	get displayLocation(): vscode.InlineCompletionDisplayLocation | undefined {
		...
		return displayLocation
			? { range: toExternalRange(displayLocation.range), label: displayLocation.label }
			: undefined
	}
...
function displayLocationEquals(
	a: INextEditDisplayLocation | undefined,
	b: INextEditDisplayLocation | undefined,
): boolean {
...
export async function getCodeActionsForDiagnostic(
	...
	const codeActions = await raceTimeout(raceCancellation(executeCodeActionProviderPromise, token), 1000)
...
export class Diagnostic {
	static fromVSCodeDiagnostic(diagnostic: vscode.Diagnostic): Diagnostic {
		return new Diagnostic(
			...
			diagnostic.code && !(typeof diagnostic.code === "number") && !(typeof diagnostic.code === "string")
