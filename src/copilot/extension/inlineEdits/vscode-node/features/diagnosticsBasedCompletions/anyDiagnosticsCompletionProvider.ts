...
export class AnyDiagnosticCompletionProvider implements IDiagnosticCompletionProvider<AnyDiagnosticCompletionItem> {
	...
	async provideDiagnosticCompletionItem(
	...
	): Promise<AnyDiagnosticCompletionItem | null> {
		for (const diagnostic of sortedDiagnostics) {
			...
			const codeActionsFixingCodeAction = availableCodeActions.filter((action) =>
				doesCodeActionFixDiagnostics(action, diagnostic),
			)
			...
			logList(
				`Found the following code action which fix \`${diagnostic.message}\``,
				codeActionsFixingCodeAction,
				logContext,
				this._tracer,
			)
