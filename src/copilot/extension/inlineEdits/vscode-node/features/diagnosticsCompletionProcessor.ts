...
import {
	...
	DiagnosticSeverity,
	distanceToClosestDiagnostic,
	IDiagnosticCompletionProvider,
	log,
	logList,
	sortDiagnosticsByDistance,
	toInternalPosition,
} from "./diagnosticsBasedCompletions/diagnosticsCompletions"
import {
	ImportDiagnosticCompletionItem,
	ImportDiagnosticCompletionProvider,
} from "./diagnosticsBasedCompletions/importDiagnosticsCompletionProvider"
...
export class DiagnosticsCollection {
	...
	applyEdit(previous: StringText, edit: StringEdit, after: StringText): boolean {
		...
		for (const diagnostic of this._diagnostics) {
			...
			let edgeCharacter
			if (isSamePrefix) {
				const offsetAfterOldRange =
					newOffsetRange.endExclusive - (newOffsetRange.length - oldOffsetRange.length)
				edgeCharacter = after.getValueOfRange(
					Range.fromPositions(
...
export class DiagnosticsCompletionProcessor extends Disposable {
	...
	constructor(
		...
		const diagnosticsExplorationEnabled = true

		const importProvider = new ImportDiagnosticCompletionProvider(
			this._tracer.sub("Import"),
			workspaceService,
			fileSystemService,
		)
		...
		this._diagnosticsCompletionProviders = derived((reader) => {
			const providers: IDiagnosticCompletionProvider[] = [importProvider, asyncProvider]
		...
		this._register(
			this._languageDiagnosticsService.onDidChangeDiagnostics(async (e) => {
		...
		this._register(
			this._tabsAndEditorsService.onDidChangeActiveTextEditor(async (e) => {
		...
		this._register(
			this._worker.onDidChange((result) => {
					...
					autorunWithChanges(
						...
						(data) => {
							for (const edit of data.value.changes) {
								...
								const hasInvalidatedRange = this._currentDiagnostics.applyEdit(
									data.value.previous,
									edit,
									data.value.value,
								)
	...
	private async _updateState(): Promise<void> {
		...
		await this._worker.schedule(async (token: CancellationToken) =>
			this._runCompletionHandler(workspaceDocument, diagnosticsSorted, availableDiagnostics, cursor, log, token),
		)
	}
	...
	private _getDiagnostics(
		...
		cursor: Position,
		logContext: DiagnosticInlineEditRequestLogContext,
	): { availableDiagnostics: Diagnostic[]; relevantDiagnostics: Diagnostic[] } {
		...
		const filterDiagnosticsAndLog = (
			diagnostics: Diagnostic[],
			message: string,
			filterFn: (diagnostics: Diagnostic[]) => Diagnostic[],
		): Diagnostic[] => {
		...
		relevantDiagnostics = filterDiagnosticsAndLog(relevantDiagnostics, "Filtered by provider", (ds) =>
			ds.filter((diagnostic) =>
				providers.some((provider) => provider.providesCompletionsForDiagnostic(diagnostic, language, cursor)),
			),
		)
		relevantDiagnostics = filterDiagnosticsAndLog(relevantDiagnostics, "Filtered by recent acceptance", (ds) =>
			ds.filter((diagnostic) => !this._hasDiagnosticRecentlyBeenAccepted(diagnostic)),
		)
		relevantDiagnostics = filterDiagnosticsAndLog(relevantDiagnostics, "Filtered by no recent edit", (ds) =>
			this._filterDiagnosticsByRecentEditNearby(ds, workspaceDocument),
		)

		return { availableDiagnostics, relevantDiagnostics }
	}

	private async _runCompletionHandler(
		workspaceDocument: IVSCodeObservableDocument,
		diagnosticsSorted: Diagnostic[],
		allDiagnostics: Diagnostic[],
		cursor: Position,
		log: DiagnosticInlineEditRequestLogContext,
		token: CancellationToken,
	): Promise<IDiagnosticsCompletionState> {
		...
		try {
			...
			completionItem = await this._getCompletionFromDiagnostics(
				workspaceDocument,
				diagnosticsSorted,
				cursor,
				log,
		...
		const allNoneSupportedDiagnostics = allDiagnostics.filter(
			(diagnostic) => !diagnosticsSorted.includes(diagnostic),
		)
		telemetryBuilder.setDistanceToUnknownDiagnostic(
			distanceToClosestDiagnostic(allNoneSupportedDiagnostics, cursor),
		)

		// Distance to the closest none result diagnostic
		const allAlternativeDiagnostics = allDiagnostics.filter(
			(diagnostic) => !completionItem || !completionItem.diagnostic.equals(diagnostic),
		)
		telemetryBuilder.setDistanceToAlternativeDiagnostic(
			distanceToClosestDiagnostic(allAlternativeDiagnostics, cursor),
		)

		if (completionItem) {
			const hasDiagnosticForSameRange = allAlternativeDiagnostics.some((diagnostic) =>
				completionItem.diagnostic.range.equalsRange(diagnostic.range),
			)
	...
	getCurrentState(docId: DocumentId): DiagnosticCompletionState {
		...
		if (!workspaceDocument) {
			return {
				item: undefined,
				telemetry: new DiagnosticsCompletionHandlerTelemetry()
					.addDroppedReason("WorkspaceDocumentNotFound")
					.build(),
		...
		if (currentState === NoResultReason.HasNotRunYet) {
			return {
				item: undefined,
				telemetry: new DiagnosticsCompletionHandlerTelemetry().build(),
				logContext: undefined,
			}
		}
		if (currentState === NoResultReason.WorkInProgress) {
			return {
				item: undefined,
				telemetry: new DiagnosticsCompletionHandlerTelemetry()
					.addDroppedReason(NoResultReason.WorkInProgress)
					.build(),
				logContext: undefined,
			}
		}
		...
		if (
			!this._isCompletionItemValid(completionItem, workspaceDocument, currentState.logContext, telemetryBuilder)
		) {
			return { item: undefined, telemetry: telemetryBuilder.build(), logContext }
		}
		...
		if (completionItem.documentId !== docId) {
			...
			return {
				item: undefined,
				telemetry: telemetryBuilder.addDroppedReason("wrong-document").build(),
				logContext,
			}
		}
	...
	private async _getCompletionFromDiagnostics(
		workspaceDocument: IVSCodeObservableDocument,
		diagnosticsSorted: Diagnostic[],
		pos: Position,
		logContext: DiagnosticInlineEditRequestLogContext,
		token: CancellationToken,
		tb: DiagnosticsCompletionHandlerTelemetry,
	): Promise<DiagnosticCompletionItem | null> {
		...
		const diagnosticsCompletionItems = await this._fetchDiagnosticsBasedCompletions(
			workspaceDocument,
			diagnosticsSorted,
			pos,
			logContext,
		...
		return (
			diagnosticsCompletionItems.find((item) =>
				this._isCompletionItemValid(item, workspaceDocument, logContext, tb),
			) ?? null
		)
	}

	private async _fetchDiagnosticsBasedCompletions(
		workspaceDocument: IVSCodeObservableDocument,
		sortedDiagnostics: Diagnostic[],
		pos: Position,
		logContext: DiagnosticInlineEditRequestLogContext,
		token: CancellationToken,
	): Promise<DiagnosticCompletionItem[]> {
		const providers = this._diagnosticsCompletionProviders.get()
	...
	public handleEndOfLifetime(
		completionItem: DiagnosticCompletionItem,
		reason: vscode.InlineCompletionEndOfLifeReason,
	): void {
		const provider = this._diagnosticsCompletionProviders
			.get()
			.find((p) => p.providerName === completionItem.providerName)
		if (!provider) {
	...
	private _isCompletionItemValid(
		...
		if (
			provider &&
			provider.isCompletionItemStillValid &&
			!provider.isCompletionItemStillValid(item, workspaceDocument)
		) {
	...
	private _isUndoRecentEdit(diagnostic: DiagnosticCompletionItem): boolean {
		...
		return diagnosticWouldUndoUserEdit(
			diagnostic,
			documentHistory.before,
			documentHistory.after,
			Edits.single(documentHistory.edits),
		)
	}

	private _filterDiagnosticsByRecentEditNearby(
		diagnostics: Diagnostic[],
		document: IVSCodeObservableDocument,
	): Diagnostic[] {
		...
		return diagnostics.filter((diagnostic) => {
			...
			const potentialIntersection = findFirstMonotonous(
				newRanges,
				(r) => r.endExclusive >= currentOffsetRange.start,
			)
...
function diagnosticWouldUndoUserEdit(
	diagnostic: DiagnosticCompletionItem,
	documentBefore: StringText,
	documentAfter: StringText,
	edits: Edits,
): boolean {
...
class AsyncWorker<T extends {}> extends Disposable {
	...
	private set _currentResult(value: T) {
		if (
			!this._taskQueue.isTriggered() &&
			(this.__currentResult === undefined || !this._equals(value, this.__currentResult))
		) {
