...
export interface INextEditProvider<T extends INextEditResult, TData = void> extends IDisposable {
	[x: string]: any;
	readonly ID: string;
	getNextEdit(docId: DocumentId, context: vscode.InlineCompletionContext, logContext: InlineEditRequestLogContext, cancellationToken: CancellationToken): Promise<T>;
...
export class DiagnosticsNextEditProvider extends Disposable implements INextEditProvider<DiagnosticsNextEditResult, boolean> {
	...
	async getNextEdit(docId: DocumentId, context: vscode.InlineCompletionContext, logContext: InlineEditRequestLogContext, cancellationToken: CancellationToken): Promise<DiagnosticsNextEditResult> {
	...
	async runUntilNextEdit(docId: DocumentId, context: vscode.InlineCompletionContext, logContext: InlineEditRequestLogContext, delayStart: number, cancellationToken: CancellationToken): Promise<DiagnosticsNextEditResult> {
		try {
	...
	private _createNextEditResult(diagnosticEditResult: DiagnosticCompletionState, logContext: InlineEditRequestLogContext): DiagnosticsNextEditResult {
