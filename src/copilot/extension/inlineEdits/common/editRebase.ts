...
function _tryRebase(
	...
	if (detailedEdits.length < originalEdits.length) {
		...
		for (let index = detailedEdits.length; index < originalEdits.length; index++) {
			...
			detailedEdits[index] = computeDiff(
				edit.replaceRange.substring(intermediateDocument),
				edit.newText,
	...
	const grouped = rebasedEdit.replacements.reduce(
		(acc, item) => {
	...
	for (let index = 0; index < grouped.length; index++) {
		...
		const newText = group
			.map((edit, i, a) => {
				if (i > 0) {
					return (
						currentDocumentContent.substring(a[i - 1].replaceRange.endExclusive, edit.replaceRange.start) +
						edit.newText
					)
				} else {
					return edit.newText
	...
	if (
		resolution === "strict" &&
		resultEdits.length > 0 &&
		new SingleEdits(originalEdits).apply(originalDocument) !==
			StringEdit.create(resultEdits.map((r) => r.rebasedEdit)).apply(currentDocumentContent)
	) {
...
export function checkEditConsistency(
	...
	current: string,
	tracer: ITracer,
	enabled = TROUBLESHOOT_EDIT_CONSISTENCY,
) {
	if (!enabled) {
		return true
	}
...
export function tryRebaseStringEdits<T extends IEditData<T>>(
	content: string,
	ours: StringEdit,
	base: StringEdit,
	resolution: "strict" | "lenient",
	...
	return tryRebaseEdits(
		content,
		ours.mapData((r) => new VoidEditData()),
		base,
		resolution,
		nesConfigs,
	)?.toStringEdit()
		...
		if (!ourEdit) {
			...
			if (!ourE.replaceRange.containsRange(baseEdit.replaceRange)) {
					...
					if (updated.endsWith(added)) {
						ourE = new AnnotatedStringReplacement(
							OffsetRange.fromTo(
								...
								ourE.replaceRange.endExclusive - added.length,
							),
				...
				// Skipping the case where there is another edit for now because we might have to merge with it first.
					...
					ourIdx === ours.replacements.length - 1 &&
					ourE.replaceRange.endExclusive < baseEdit.replaceRange.endExclusive
				) {
					...
					if (updated.startsWith(added)) {
						ourE = new AnnotatedStringReplacement(
							OffsetRange.fromTo(
								ourE.replaceRange.start + added.length,
								baseEdit.replaceRange.endExclusive,
							),
			...
			if (ourE.replaceRange.intersectsOrTouches(baseEdit.replaceRange)) {
				if (
					ourE.replaceRange.containsRange(baseEdit.replaceRange) &&
					ourE.newText.length >= baseEdit.newText.length
				) {
					let delta = 0
					...
					while (baseE && ourE.replaceRange.containsRange(baseE.replaceRange)) {
						ourNewTextOffset = agreementIndexOf(
							content,
							ourE,
							baseE,
							previousBaseE,
					...
					newEdits.push(
						new AnnotatedStringReplacement(
							new OffsetRange(
								ourE.replaceRange.start + offset,
								ourE.replaceRange.endExclusive + offset + delta,
							),
							ourE.newText,
							ourE.data,
						),
				...
				newEdits.push(
					new AnnotatedStringReplacement(ourEdit.replaceRange.delta(offset), ourEdit.newText, ourEdit.data),
				)
				ourIdx++
			} else {
				if (resolution === "strict") {
...
function agreementIndexOf<T extends IEditData<T>>(
	content: string,
	ourE: AnnotatedStringReplacement<T>,
	baseE: StringReplacement,
	previousBaseE: StringReplacement | undefined,
