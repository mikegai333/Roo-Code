...
;((Event2) => {
	...
	function map(event, map2, disposable) {
		return snapshot(
			(listener, thisArgs = null, disposables) =>
				event((i) => listener.call(thisArgs, map2(i)), null, disposables),
			disposable,
		)
	}
	Event2.map = map
	...
	function filter(event, filter2, disposable) {
		return snapshot(
			(listener, thisArgs = null, disposables) =>
				event((e) => filter2(e) && listener.call(thisArgs, e), null, disposables),
			disposable,
		)
	}
	...
	function split(event, isT, disposable) {
		return [Event2.filter(event, isT, disposable), Event2.filter(event, (e) => !isT(e), disposable)]
	}
	Event2.split = split
...
var AmbiguousCharacters = class _AmbiguousCharacters {
	...
	static {
		this._locales = new Lazy(() =>
			Object.keys(_AmbiguousCharacters.ambiguousCharacterData.value).filter((k) => !k.startsWith("_")),
		)
	}
	static getLocales() {
	...
	static containsInvisibleCharacter(str) {
		for (let i = 0; i < str.length; i++) {
			...
			if (
				...
				(_InvisibleCharacters.isInvisibleCharacter(codePoint) || codePoint === 32) /* space */
			) {
				return true
			}
		}
	...
	compose(other) {
		...
		for (const r2 of edits2.replacements) {
			...
			if (!firstIntersecting) {
				...
				const suffixLength =
					lastIntersecting.replaceRange.endExclusive + edit1ToEdit2 - r2.replaceRange.endExclusive
				if (suffixLength > 0) {
...
var AnnotatedStringEdit = class _AnnotatedStringEdit extends BaseStringEdit {
	...
	static insert(offset, replacement, data) {
		return new _AnnotatedStringEdit([
			new AnnotatedStringReplacement(OffsetRange.emptyAt(offset), replacement, data),
		])
	}
...
var AnnotatedStringReplacement = class _AnnotatedStringReplacement extends BaseStringReplacement {
	...
	equals(other) {
		return (
			this.replaceRange.equals(other.replaceRange) && this.newText === other.newText && this.data === other.data
		)
	}
	tryJoinTouching(other) {
		...
		return new _AnnotatedStringReplacement(
			this.replaceRange.joinRightTouching(other.replaceRange),
			this.newText + other.newText,
			joined,
		)
...
var TextEdit = class _TextEdit {
	...
	normalize() {
		...
		for (const r of this.replacements) {
			if (
				...
				replacements[replacements.length - 1].range.getEndPosition().equals(r.range.getStartPosition())
			) {
	...
	inverse(doc) {
		...
		return new _TextEdit(
			this.replacements.map((e, idx) => new TextReplacement(ranges[idx], doc.getValueOfRange(e.range))),
		)
	}
	getNewRanges() {
		const newRanges = []
...
var TextReplacement = class _TextReplacement {
	...
	removeCommonPrefix(text) {
		...
		const start = TextLength.ofText(normalizedOriginalText.substring(0, commonPrefixLen)).addToPosition(
...
var LineBasedText = class extends AbstractText2 {
...
var StringText = class extends AbstractText2 {
	constructor(value) {
		super()
		this.value = value
...
function lineRangeMappingFromRangeMappings(alignments, originalLines, modifiedLines, dontAssertStartLine = false) {
	...
	assertFn(() => {
		if (!dontAssertStartLine && changes.length > 0) {
			...
			if (
				modifiedLines.length.lineCount - changes[changes.length - 1].modified.endLineNumberExclusive !==
		...
		return checkAdjacentItems(
			changes,
			(m1, m2) =>
				m2.original.startLineNumber - m1.original.endLineNumberExclusive ===
					m2.modified.startLineNumber - m1.modified.endLineNumberExclusive && // There has to be an unchanged line in between (otherwise both diffs should have been joined)
...
var DiffAlgorithmResult = class _DiffAlgorithmResult {
	...
	static trivial(seq1, seq2) {
		return new _DiffAlgorithmResult(
...
var SequenceDiff = class _SequenceDiff {
	...
	intersectsOrTouches(other) {
		return (
			this.seq1Range.intersectsOrTouches(other.seq1Range) || this.seq2Range.intersectsOrTouches(other.seq2Range)
		)
	}
	intersect(other) {
...
var LinesSliceCharSequence = class {
	...
	extendToFullLines(range) {
		...
		const end =
			findFirstMonotonous(this.firstElementOffsetByLineIdx, (x) => range.endExclusive <= x) ??
...
function isWordChar(charCode) {
	return (
		...
		(charCode >= 48 /* Digit0 */ && charCode <= 57) /* Digit9 */
	)
}
		...
		for (let i = change.modified.startLineNumber; i < change.modified.endLineNumberExclusive - 2; i++) {
			...
			original3LineHashes.forEach(key, ({ range }) => {
				for (const lastMapping of lastMappings) {
					if (
						lastMapping.originalLineRange.endLineNumberExclusive + 1 === range.endLineNumberExclusive &&
	...
	for (let i = 0; i < moves.length; i++) {
		...
		if (extendToTop > 0) {
			originalSet.addRange(
				new LineRange(move.original.startLineNumber - extendToTop, move.original.startLineNumber),
			)
			modifiedSet.addRange(
				new LineRange(move.modified.startLineNumber - extendToTop, move.modified.startLineNumber),
			)
		}
		let extendToBottom
...
function extendDiffsToEntireWordIfAppropriate(sequence1, sequence2, sequenceDiffs, findParent, force = false) {
	...
	function scanWord(pair, equalMapping) {
		...
		if (
			(force && equalChars1 + equalChars2 < w.seq1Range.length + w.seq2Range.length) ||
	...
	do {
		...
		for (let i = 1; i < diffs.length; i++) {
			let shouldJoinDiffs2 = function (before, after) {
				...
				if (
					Math.pow(
						Math.pow(cap(beforeLineCount1 * 40 + beforeSeq1Length), 1.5) +
							Math.pow(cap(beforeLineCount2 * 40 + beforeSeq2Length), 1.5),
						1.5,
					) +
						Math.pow(
							Math.pow(cap(afterLineCount1 * 40 + afterSeq1Length), 1.5) +
								Math.pow(cap(afterLineCount2 * 40 + afterSeq2Length), 1.5),
							1.5,
						) >
					(max ** 1.5) ** 1.5 * 1.3
				) {
...
var DefaultLinesDiffComputer = class {
	...
	computeDiff(originalLines, modifiedLines, options) {
		...
		const lineAlignmentResult = (() => {
			if (sequence1.length + sequence2.length < 1700) {
				return this.dynamicProgrammingDiffing.compute(sequence1, sequence2, timeout, (offset1, offset2) =>
	...
	computeMoves(
		...
		const movesWithDiffs = moves.map((m) => {
			const moveChanges = this.refineDiff(
				originalLines,
				modifiedLines,
				new SequenceDiff(m.original.toOffsetRange(), m.modified.toOffsetRange()),
				timeout,
				considerWhitespaceChanges,
				options,
			)
	...
	refineDiff(originalLines, modifiedLines, diff, timeout, considerWhitespaceChanges, options) {
		...
		const result = diffs.map(
			(d) => new RangeMapping(slice1.translateRange(d.seq1Range), slice2.translateRange(d.seq2Range)),
		)
		if (check) {
