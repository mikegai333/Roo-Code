...
import {
	...
	LineRangeMapping,
	RangeMapping,
} from "../../../util/vs/editor/common/diff/rangeMapping"
...
export class DiffServiceImpl implements IDiffService {
	...
	async computeDiff(original: string, modified: string, options: ILinesDiffComputerOptions): Promise<IDocumentDiff> {
		console.log("===diff===")
		console.log(path.join(__dirname, "diffWorker.js"))
		console.log(path.join(__dirname, "../../../../dist/diffWorker.js"))
	...
	return changes.map(
		(c) =>
			new DetailedLineRangeMapping(
				...
				c[4]?.map(
					(c) => new RangeMapping(new Range(c[0], c[1], c[2], c[3]), new Range(c[4], c[5], c[6], c[7])),
				),
			),
	)
}
