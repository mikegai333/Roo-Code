...
export function requestHasNotebookRefs(
	request: ChatRequest,
	notebookService: INotebookService,
	options?: { checkPromptAsWell: boolean },
): boolean {
...
export function parseAndCleanStack(jsonString: string): string {
	try {
		...
		return (
			removeAnsiEscapeCodes(parsed?.stack || parsed.message || "") || parsed.message || parsed.name || jsonString
		)
	} catch {
