...
import { EndOfLine, NotebookCellKind } from "vscode"
...
class AlternativeTextDocument extends AlternativeNotebookDocument {
	override fromCellPosition(cellIndex: number, position: Position): Position {
		...
		const offset =
			alternativeContentText.indexOf(cellMarker) + cellMarker.length + eolLength + markdownOffset + offsetInCell
...
export class AlternativeTextNotebookContentProvider extends BaseAlternativeNotebookContentProvider {
	...
	public override getSummaryOfStructure(
		notebook: NotebookDocument,
		cellsToInclude: NotebookCell[],
		existingCodeMarker: string,
	): string {
		...
		notebook.getCells().forEach((cell) => {
			if (cellsToInclude.includes(cell)) {
				...
				if (cellSummary.source.length && cellSummary.source[0].trim().length) {
					...
					cellSummary.source = [
						existingCodeMarkerWithComment,
						cellSummary.source.filter((line) => line.trim().length)[0],
						existingCodeMarkerWithComment,
					]
				} else {
	...
	public override async *parseAlternateContent(
		notebookOrUri: NotebookDocument | Uri,
		inputStream: AsyncIterable<LineOfText>,
		token: CancellationToken,
	): AsyncIterable<LineOfCellText> {
		...
		for await (const lineOfText of inputStream) {
			...
			const isLineCommentForEmptyCellWithoutCellMarker =
				line.startsWith(`${lineCommentStart}%% [`) && line.trimEnd().endsWith("]")
			...
			const cellParts =
				isLineCommentWithCellMarker || isLineCommentForEmptyCellWithoutCellMarker
					? extractCellParts(line, defaultLanguage)
					: undefined
			if ((isLineCommentWithCellMarker || isLineCommentForEmptyCellWithoutCellMarker) && cellParts?.language) {
				...
				const lineOfCellText: LineOfCellText & { emitted: Boolean } = {
					index: -1,
					uri: undefined,
					language: undefined,
				...
				lineOfCellText.kind =
					cell?.kind ||
					(lineOfCellText.language === "markdown" ? NotebookCellKind.Markup : NotebookCellKind.Code)
...
function generateAlternativeCellContent(
	cell: SummaryCell,
