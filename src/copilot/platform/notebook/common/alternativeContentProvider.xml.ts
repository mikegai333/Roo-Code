...
import { EndOfLine, NotebookCellKind, Position } from "vscode"
...
export class AlternativeXmlNotebookContentProvider extends BaseAlternativeNotebookContentProvider {
	...
	public getAlternativeContent(notebook: NotebookDocument): string {
		...
		return cells
			.map((cell) => `${generateCellMarker(cell)}${EOL}${cell.source.join(EOL)}${EOL}${EndDelimter}`)
			.join(EOL)
	}
	...
	public stripCellMarkers(text: string): string {
		...
		if (lines.length && lines[lines.length - 1].trim().endsWith(EndDelimter)) {
			lines[lines.length - 1] = lines[lines.length - 1].substring(
				0,
				lines[lines.length - 1].lastIndexOf(EndDelimter),
			)
		}
	...
	public async *parseAlternateContent(
		...
		for await (const lineOfText of inputStream) {
			...
			if (
				(line.startsWith(StartDelimter) || line.startsWith(StartEmptyCellDelimter)) &&
				(index < 0 || endDelimiterSeen || (previousLineEndedWithEndCellMarker && previousLine))
			) {
				...
				const lineOfCellText: LineOfCellText = {
					type: "start",
					index,
					uri: undefined,
					language: undefined,
				...
				const cell =
					cellIdMap.get(cellParts.id)?.document.languageId === cellParts.language
						? cellIdMap.get(cellParts.id)
						: undefined
