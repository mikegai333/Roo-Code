...
export class PositionOffsetTransformer {
	...
	private _acceptInsertText(position: vscode.Position, insertText: string): void {
		...
		this._setLineText(position.line, this._lines[position.line].substring(0, position.character) + insertLines[0])
	...
	toOffsetRange(range: Range): OffsetRange {
		return new OffsetRange(this.getOffset(range.start), this.getOffset(range.end))
	}
