...
export class TextDocumentSnapshot {
	...
	static fromJSON(doc: TextDocument, json: ITextDocumentSnapshotJSON): TextDocumentSnapshot {
		return new TextDocumentSnapshot(doc, URI.from(json.uri), json._text, json.languageId, json.eol, json.version)
	}
	...
	getWordRangeAtPosition(_position: Position): Range | undefined {
		...
		const wordAtText = getWordAtText(position.character + 1, DEFAULT_WORD_REGEXP, this.lines[position.line], 0)
