...
export function stringEditFromTextContentChange(contentChanges: readonly vscode.TextDocumentContentChangeEvent[]) {
	const editsArr = contentChanges.map(
		(c) => new StringReplacement(OffsetRange.ofStartAndLength(c.rangeOffset, c.rangeLength), c.text),
	)
	editsArr.reverse()
