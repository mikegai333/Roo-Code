...
export interface RepoContext {
	...
	readonly changes:
		| { mergeChanges: Change[]; indexChanges: Change[]; workingTree: Change[]; untrackedChanges: Change[] }
		| undefined
...
export function getGithubRepoIdFromFetchUrl(fetchUrl: string): GithubRepoId | undefined {
	...
	const matchedHost = topLevelUrls.find(
		(topLevelUrl) => parsed.host === topLevelUrl || parsed.host.endsWith("." + topLevelUrl),
	)
	if (!matchedHost) {
		return
	}
