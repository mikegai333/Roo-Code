...
export namespace ResponseProcessor {
	...
	export async function* diff(
		...
		modifiedLines: AsyncIterableObject<string>,
	...
	function checkForConvergence(
		...
		if (
			nNonSigMatches > 0 &&
			match[0] - state.startLineIdx === state.newLines.length - 1 /* to discount for converging line */
		) {
		...
		for (; newLinesIdx >= 0; --newLinesIdx) {
			...
			candidates = candidates.filter(
				([_, currentIdx]) => originalLines[currentIdx] === state.newLines[newLinesIdx],
			)
		...
		const zeroBasedLineRange: [startLineOffset: number, endLineOffset: number] = [
			state.startLineIdx,
			originalLinesConvIdx,
		]
		const lineRange = new LineRange(zeroBasedLineRange[0] + 1, zeroBasedLineRange[1] + 1)
