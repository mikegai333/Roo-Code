...
export function chainStatelessNextEditProviders(
	base: IStatelessNextEditProvider,
	...decorators: ((provider: IStatelessNextEditProvider) => IStatelessNextEditProvider)[]
): IStatelessNextEditProvider {
...
export abstract class EditFilterAspect implements IStatelessNextEditProvider {
	get ID(): string {
		return this._baseProvider.ID
	}

	constructor(private readonly _baseProvider: IStatelessNextEditProvider) {}
	...
	abstract filterEdit(
		resultDocument: StatelessNextEditDocument,
		singleEdits: readonly LineReplacement[],
	): readonly LineReplacement[]
}
...
export class IgnoreTriviaWhitespaceChangesAspect extends EditFilterAspect {
	override filterEdit(
		...
		const filteredEdits = singleEdits.filter(
			(e) => !this._isWhitespaceOnlyChange(e, resultDocument.documentAfterEditsLines),
		)
