...
export class NesXtabHistoryTracker extends Disposable {
	...
	constructor(
		...
		private readonly maxHistorySize = NesXtabHistoryTracker.MAX_HISTORY_SIZE,
	) {
		super()
	...
	private handleVisibleRangesChange(doc: IObservableDocument, visibleRangesChange: DocumentSelectionChangedEvent) {
		...
		const entry: IXtabHistoryEntry = {
			docId: doc.id,
			kind: "visibleRanges",
			visibleRanges: visibleRangesChange.value,
			documentContent: doc.value.get(),
		}
	...
	private handleEdits(doc: IObservableDocument, rootedEdits: DocumentChangedEvent) {
		...
		if (
			!currentLineEdit.isEmpty() &&
			!lastLineEdit.isEmpty() &&
			lastLineEdit.edits[0].lineRange.startLineNumber === currentLineEdit.edits[0].lineRange.startLineNumber
		) {
			// merge edits
			previousRecord.removeFromHistory()
