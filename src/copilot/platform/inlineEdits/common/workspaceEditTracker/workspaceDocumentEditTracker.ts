...
class DocumentEditHistory {
	...
	public handleEdit(edit: StringEdit, isInCooldown: boolean): void {
		...
		const { e1: newRecentEdits, e2: oldEdits } = updatedRecentEdits.decomposeSplit(
			(r) => r.data.value > this._documentStateID - this._historyLength,
		)
	...
	public getNRecentEdits(n: number): DocumentHistoryDifference {
		const { e1: nRecentEdits, e2: oldEdits } = this._recentEdits.decomposeSplit(
			(r) => r.data.value > this._documentStateID - n,
		)
