...
export class ObservableGit extends Disposable {
	...
	constructor(@IGitExtensionService private readonly _gitExtensionService: IGitExtensionService) {
		super()

		this._gitApi = observableFromEvent(
			this,
			(listener) => this._gitExtensionService.onDidChange(listener),
			() => this._gitExtensionService.getExtensionApi(),
		)
	...
	async init() {
		...
		const repos = observableFromEvent(
			this,
			(e) => gitApi.onDidOpenRepository(e),
			() => gitApi.repositories,
		)
		...
		mapObservableArrayCached(
			...
			repos,
			(repo, store) => {
				const stateChangeObservable = observableFromEvent(
					(listener) => repo.state.onDidChange(listener),
					() => repo.state.HEAD?.name,
				)
				store.add(
					autorunWithStore((reader, _store) => {
