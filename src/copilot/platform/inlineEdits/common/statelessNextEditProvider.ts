...
export type PushEdit = (
	edit: Result<{ edit: LineReplacement; window?: OffsetRange; targetDocument?: DocumentId }, NoNextEditReason>,
) => void
...
export class StatelessNextEditTelemetryBuilder {
	...
	public build(result: Result<void, NoNextEditReason>): IStatelessNextEditTelemetry {
		...
		const prompt = this._prompt
			? JSON.stringify(this._prompt.map(({ role, content }) => ({ role, content })))
			: undefined
		...
		if (result.isError()) {
			if (
				result.err instanceof NoNextEditReason.ActiveDocumentHasNoEdits ||
	...
	public setResponse(response: Promise<{ ttft: number | undefined; response: any }>): this {
