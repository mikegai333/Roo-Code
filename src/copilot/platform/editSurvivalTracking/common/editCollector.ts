...
export class EditCollector implements IEditCollector {
	...
	public addEdits(edits: vscodeTypes.TextEdit[]): void {
		console.log("===[EditCollector] Adding edits", edits)
		this._document.applyTextEdits(edits)
	}

	public getText(): string {
		console.log("===[EditCollector] Getting current text of length", this._document.getValue().length)
		return this._document.getValue()
	}

	public async getEdits(): Promise<StringEdit> {
		console.log("===[EditCollector] Computing edits since initial text")
...
export class OffsetBasedTextDocument {
	...
	applyTextEdits(edits: vscodeTypes.TextEdit[]) {
		console.log("===[OffsetBasedTextDocument] Applying text edits", edits)
	...
	applyOffsetEdit(edit: StringEdit): void {
		console.log("===[OffsetBasedTextDocument] Applying offset edit", edit.toString())
		...
		if (line + 1 < this._converter.lines) {
			...
			if (endLineOffest > lineOffet) {
				...
				if (
					...
					this._value.charCodeAt(endLineOffest - 1) === CharCode.CarriageReturn
				) {
					endLineOffest--
				}
			} else {
