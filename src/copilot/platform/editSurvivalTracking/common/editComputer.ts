...
export class DocumentEditRecorder extends Disposable {
	...
	constructor(
		...
		this._register(
			this._workspaceService.onDidChangeTextDocument((e) => {
				console.log("===[DocumentEditRecorder] Document changed", e.document.uri.toString())
	 ...
	 */
	getEdits(): StringEdit {
		console.log("===[DocumentEditRecorder] Getting edits since initial version", this.initialTextVersion)
		if (this._store.isDisposed) {
