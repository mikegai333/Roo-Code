...
export class EditSurvivalReporter {
	...
	private readonly _arcTracker =
		this._options.includeArc === true
			? new ArcTracker(this._documentTextBeforeMarkedEdits, this._markedEdits)
			: undefined
	...
	constructor(
		...
		this._store.add(
			workspaceService.onDidChangeTextDocument((e) => {
				console.log("===[EditSurvivalReporter] Document changed", e.document.uri.toString())
		...
		console.log("===[EditSurvivalReporter] Initialized with initial branch name:", this._initialBranchName)
	...
	private _reportAfter(timeoutMs: number, cb?: () => void) {
		console.log("===[EditSurvivalReporter] Scheduling report in", timeoutMs, "ms")
	...
	private _report(timeMs: number): void {
		//**调用computeTrackedEditsSurvivalScore计算函数 */
		console.log("===发送报告===")
