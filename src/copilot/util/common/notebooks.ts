...
export function getNotebookCellOutput(
	outputUri: Uri,
	notebookDocuments: readonly vscode.NotebookDocument[],
): [vscode.NotebookDocument, vscode.NotebookCell, vscode.NotebookCellOutput] | undefined {
...
export function getNotebookAndCellFromUri(
	uri: Uri,
	notebookDocuments: readonly vscode.NotebookDocument[],
): [undefined, undefined] | [vscode.NotebookDocument, vscode.NotebookCell | undefined] {
...
export function isDocumentExcludePattern(
	pattern: string | vscode.RelativePattern | INotebookExclusiveDocumentFilter | INotebookFilenamePattern,
	...
	return (
		typeof arg === "object" && arg !== null && (typeof arg.include === "string" || isRelativePattern(arg.include))
	)
}
	...
	if (isFilenamePattern(selector)) {
		...
		if (glob.match(selector.filenamePattern, basename(resource.fsPath).toLowerCase())) {
			if (
				...
				glob.match(selector.excludeFileNamePattern, basename(resource.fsPath).toLowerCase())
			) {
				return false
			}
			return true
...
export function _hasSupportedNotebooks(
	...
	const validNotebookEditorContribs: INotebookEditorContribution[] = notebookEditorContributions.filter(
		(notebookEditorContrib) =>
			notebookEditorContrib.selector.some((selector) => notebookSelectorMatches(uri, selector)),
	)
	...
	for (const association of validAssociations) {
		if (
			validNotebookEditorContribs.some(
				(notebookEditorContrib) => notebookEditorContrib.type === association.viewType,
			)
		) {
			return true
		}
	...
	if (
		validNotebookEditorContribs.some(
			(notebookEditorContrib) =>
				(notebookEditorContrib.priority ?? RegisteredEditorPriority.default) ===
